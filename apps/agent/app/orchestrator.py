...
class Run:
    ...
    # Blocked invoices a person turned down, accumulated the same way and for the
    # same reason: rejection now arrives across as many calls as approval does,
    # and a row rejected in an earlier call must not still count as open.
...
async def _prepare(
    ...
    """
    async with gate:
        # The order goes first and alone, because SAP's own id for this supplier
        # is on it. Asking about the number printed on the invoice searches for a
        # party SAP has never heard of, so the duplicate lookup below could only
        # ever come back clean - which made rule 16, the one check that stops a
        # batch being parked twice, a false pass.
        po = await sap.purchase_order(inv.purchase_order, inv.purchase_order_item)
        if po and po.supplier:
            inv.sap_supplier = po.supplier

        # Both of these need the order, and neither needs the other, so the pair
        # still costs one round trip rather than two.
        inv.existing_reference, gr = await asyncio.gather(
            sap.reference_exists(inv.party(), inv.reference),
            sap.goods_receipt(inv.purchase_order, inv.purchase_order_item) if po else _no_receipt(),
        )
...
async def _no_receipt() -> GoodsReceipt | None:
    """The same, for the receipt that a missing order means we never fetch."""
    return None
...
async def post_run(
    ...
    every approved invoice still outstanding. Either way, an invoice already in
    ...
    # Remembered on the run, because approval now arrives across several calls
    # and a row turned down in an earlier one must stay turned down in the next.
    run.rejected.extend(i for i in rejected if i not in run.rejected)
    ...
    for index, inv in enumerate(run.invoices):
        if inv.invoice_id not in posting or inv.invoice_id in run.parked:
            continue
        # Empty means every outstanding invoice, not none of them - the browser
        # sends `[]` for "approve all", and reading that as a narrowing to
        # nothing would park nothing while reporting success.
        if ids and inv.invoice_id not in ids:
            continue
    ...
    # Spoken on every call, not only the one that closes the run - a call that
    # refuses everything never reaches a closing summary, and a refusal has to be
    # in the sentence regardless. Reporting "0 parked" and stopping describes a
    # batch where nothing needed posting, which is the opposite of one where SAP
    # turned everything down, and it is the last line the user reads before
    # deciding the run went fine.
    parts = [f"{len(run.parked)} parked documents in SAP, fiscal year 2025."]
    if refused:
        parts.append(
    ...
    # five must leave the other four still awaiting their own approve calls. A
    # refused invoice is still outstanding, so a refusal holds the run open for
    # the retry rather than closing it as if the batch were settled.
    outstanding = [i for i in run.ready if i not in run.parked]
    if outstanding:
        run.state = "awaiting-approval"
    else:
        run.state = "done"
        db.save_run(run)  # once more, to record the closing state
        still_open = len([i for i in run.blocked if i not in run.parked and i not in run.rejected])
        if still_open:
