          ...
          >
            ...
            <div className="reveal" data-open={isOpen}>
              <div>
                ...
                {invoice.filed ? (
                  <p className="mx-5 mb-4 text-[15px] text-on-surface-variant">
                ...
                {invoice.headline ? (
                  <div className="mx-5 mb-5 rounded-[10px] border border-outline-variant bg-surface-container-low p-5">
                    ...
                    {/* Each of these asks the agent a real question. They were
                        ...
                        The two marking buttons are the exception: they ask
                        nothing and post nothing, they mark the row, and the
                        single Approve press carries every mark at once. */}
                    ...
                    {invoice.decision ? (
                      <p className="mt-3 max-w-[70ch] text-[15px] text-on-surface-variant">
...
/** Filled while the mark is on, so a marked row reads at a glance. */
const PRESSED_TONE = {
  override: "border-success/45 bg-success-container text-on-success-container",
  reject: "border-error/45 bg-error-container text-on-error-container",
} as const;
...
function Action({
  ...
  onClick: () => void;
  disabled?: boolean;
  /** Set only by the marking buttons; the question buttons have no on-state. */
  pressed?: boolean;
  ...
  return (
    <button
      ...
      onClick={onClick}
      disabled={disabled}
      aria-pressed={tone ? Boolean(pressed) : undefined}
      className={`state-layer pressable min-h-[40px] cursor-pointer rounded-[8px] border px-4 text-[15px] transition-colors disabled:cursor-default disabled:opacity-50 focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-1 focus-visible:outline-primary ${
        pressed && tone
          ? PRESSED_TONE[tone]
          : "border-outline bg-surface-container-high text-on-surface-variant hover:border-on-surface-variant hover:text-on-surface"
      }`}
    >
      {label}
    </button>
